...
export default function TeacherDashboard() {
  ...
  const selectedStudentName = formatPerson(students.find((student) => student._id === selectedGradeUserId));

  function closeModal(name) {
    setModalState((current) => ({ ...current, [name]: false }));
  }

  const invalidateTeacher = () => queryClient.invalidateQueries({ queryKey: ["teacher"] });
  ...
  const gradeCreateMutation = useMutation({
    ...
    onSuccess: (_, payload) => {
      ...
      setSelectedGradeUserId(payload.userId || "");
      closeModal("grades");
      invalidateTeacher();
    },
  ...
  const sections = [
        ...
        <div className="stack">
          <StatStrip
            items={[
              ...
              { label: "Grades", value: gradeRecords.length },
            ]}
          />
        </div>
      ),
    },
    {
      key: "groups",
      label: "Groups",
      note: "Students, attendance, grades, homework",
      description: "One department for student work, attendance, grades and homework.",
      render: () => (
        <div className="stack">
          <SectionCard title="Students" subtitle="Group student directory">
            <ListToolbar
              value={studentSearch}
          ...
          <div className="dashboard-grid dashboard-grid--dense">
            ...
            >
              <p className="muted">Keep attendance inside the groups department, one student and one lesson at a time.</p>
            </SectionCard>
            <SectionCard
              title="Homework"
            ...
            >
              <p className="muted">Homework for students in the current groups workflow.</p>
            </SectionCard>
          </div>
          <SectionCard
            title="Grades"
            subtitle="Review and edit student grades"
                  ...
                  render: (row) => (
                    editingGradeId === row._id ? (
                      <div className="form-row">
                        <input type="number" min="0" value={editingGradeScore} onChange={(event) => setEditingGradeScore(event.target.value)} placeholder="Score" />
                        <button className="button button--ghost" type="button" disabled={gradeUpdateMutation.isPending} onClick={() => gradeUpdateMutation.mutate({ id: row._id, payload: { score: Number(editingGradeScore) } })}>Save</button>
                        <button className="button button--ghost" type="button" onClick={() => { setEditingGradeId(""); setEditingGradeScore(""); }}>Cancel</button>
                      </div>
                    ) : (
                      <button className="button button--ghost" type="button" onClick={() => { setEditingGradeId(row._id); setEditingGradeScore(String(row.score ?? "")); }}>Edit</button>
                    )
                  ),
                },
              ]}
    ...
    },
    {
      ...
      render: () => (
        <SectionCard
          ...
          subtitle="Admin and teacher notification endpoint"
        ...
        >
          <p className="muted">Use notifications for payment reminders, attendance issues and homework follow-up.</p>
        </SectionCard>
      ),
    },
  ];
  ...
  return (
    <AppShell
      ...
      actions={<button className="button" type="button" onClick={invalidateTeacher}>{t("common.refreshAll", "Refresh all")}</button>}
    >
      ...
      <FormModal open={modalState.attendance} title="Mark attendance" subtitle="One record per request" onClose={() => closeModal("attendance")}>
        <form className="form-grid" onSubmit={(event) => { event.preventDefault(); attendanceMutation.mutate(attendanceDraft); }}>
          <div className="form-row">
            ...
            <input type="datetime-local" value={attendanceDraft.date} onChange={(event) => setAttendanceDraft((current) => ({ ...current, date: event.target.value }))} />
          ...
          <button className="button" type="submit" disabled={attendanceMutation.isPending}>{attendanceMutation.isPending ? "Saving..." : "Save attendance"}</button>
        </form>
      </FormModal>

      <FormModal open={modalState.homework} title="Assign homework" subtitle="Split tasks by line" onClose={() => closeModal("homework")}>
        <form className="form-grid" onSubmit={(event) => { event.preventDefault(); homeworkMutation.mutate({ ...homeworkDraft, tasks: splitTasks(homeworkDraft.tasks) }); }}>
          <div className="form-row">
            <select value={homeworkDraft.userId} onChange={(event) => setHomeworkDraft((current) => ({ ...current, userId: event.target.value }))}>
              <option value="">Select student</option>
              {students.map((student) => <option key={student._id} value={student._id}>{formatPerson(student)}</option>)}
            </select>
            <input type="datetime-local" value={homeworkDraft.date} onChange={(event) => setHomeworkDraft((current) => ({ ...current, date: event.target.value }))} />
          </div>
          <textarea value={homeworkDraft.tasks} onChange={(event) => setHomeworkDraft((current) => ({ ...current, tasks: event.target.value }))} placeholder={"Task 1\nTask 2"} />
          <button className="button" type="submit" disabled={homeworkMutation.isPending}>{homeworkMutation.isPending ? "Saving..." : "Create homework"}</button>
        </form>
      </FormModal>

      <FormModal open={modalState.grades} title="Add grade" subtitle="Direct scoring input" onClose={() => closeModal("grades")}>
        <form className="form-grid" onSubmit={(event) => { event.preventDefault(); gradeCreateMutation.mutate({ ...gradeDraft, score: Number(gradeDraft.score) }); }}>
          <div className="form-row">
            <select value={gradeDraft.userId} onChange={(event) => { const nextUserId = event.target.value; setGradeDraft((current) => ({ ...current, userId: nextUserId })); setSelectedGradeUserId(nextUserId); }}>
              <option value="">Select student</option>
              {students.map((student) => <option key={student._id} value={student._id}>{formatPerson(student)}</option>)}
            </select>
            <input value={gradeDraft.subject} onChange={(event) => setGradeDraft((current) => ({ ...current, subject: event.target.value }))} placeholder="Subject" />
            <input type="number" min="0" value={gradeDraft.score} onChange={(event) => setGradeDraft((current) => ({ ...current, score: event.target.value }))} placeholder="Score" />
          </div>
          <button className="button" type="submit" disabled={gradeCreateMutation.isPending}>{gradeCreateMutation.isPending ? "Saving..." : "Create grade"}</button>
        </form>
      </FormModal>

      <FormModal open={modalState.notifications} title="Send notification" subtitle="Direct user notification" onClose={() => closeModal("notifications")}>
        <form className="form-grid" onSubmit={(event) => { event.preventDefault(); notificationMutation.mutate(notificationDraft); }}>
          <div className="form-row">
          ...
          <textarea value={notificationDraft.message} onChange={(event) => setNotificationDraft((current) => ({ ...current, message: event.target.value }))} placeholder="Message text" />
          <button className="button" type="submit" disabled={notificationMutation.isPending}>{notificationMutation.isPending ? "Sending..." : "Send notification"}</button>
        </form>
      </FormModal>
    </AppShell>
  );
